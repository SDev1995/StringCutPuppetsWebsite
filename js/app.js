function renderGigCard(gig) {
    ...
    const ticketLink = gig.ticketUrl
        ? `<a class="button button-secondary" href="${gig.ticketUrl}" target="_blank" rel="noopener">Tickets</a>`
...
async function loadAllGigs() {
    const gigList = document.getElementById("all-gigs");
    ...
    try {
        ...
        const upcomingGigs = gigs
            .filter((gig) => new Date(`${gig.date}T23:59:59`) >= new Date())
            .sort((a, b) => new Date(a.date) - new Date(b.date));
